...
from modules.utils import ExitRetool, Font, download, eprint, pattern2string, printwrap, regex_test
...
class CloneListTools:
                ...
                if not (
                    ...
                    if title_1.clonelist_priority < title_2.clonelist_priority:
                        if report_on_match:
                            ...
                            eprint(
                                f'{Font.disabled}- Removing: ({title_2.clonelist_priority}) {title_2.full_name}{Font.end}'
                            )
                        ...
                        if report_on_match:
                            ...
                            eprint(
                                f'{Font.disabled}- Removing: ({title_1.clonelist_priority}) {title_1.full_name}{Font.end}'
                            )
                            eprint(f'\n{Font.disabled}Press enter to continue{Font.end}')
                            input()
            ...
            for mia in input_dat.clone_list.mias:
                if not isinstance(mia, str):
                    if config.user_input.verbose:  # type: ignore[unreachable]
                        warning_given = True
                        printwrap(
                            f'{Font.warning}* The following MIA title entry isn\'t a string and will be skipped:',
                            'error',
                        )
                            ...
                            if variant_title['ignore']:
                                if title in processed_titles[title.group_name]:
                                    if report_on_match:
                                        ...
                                        eprint(
                                            f'{Font.disabled}- Remove: {title.full_name}{Font.end}'
                                        )
                                        eprint(
                                            f'\n{Font.disabled}Press enter to continue{Font.end}'
                                        )
                                        ...
                                        for filter_condition in variant_title['filters']:
                                            ...
                                            if 'regionOrder' in filter_condition['conditions']:
                                                ...
                                                if (
                                                    ...
                                                    regions: list[str] = filter_condition[
                                                        'conditions'
                                                    ]['regionOrder']['higherRegions']
                                                else:
                                                    printwrap(
                                                    ...
                                                    eprint(
                                                        f'\n  {clone_list_group_contents["searchTerm"]}{Font.end}'
                                                    )
                                            ...
                                            if (
                                                ...
                                                if 'categories' in filter_condition['results']:
                                                    ...
                                                    new_title.categories = filter_condition[
                                                        'results'
                                                    ]['categories']
    ...
    def update_clonelists_metadata(
        ...
        def get_updates(
            local_path: str, update_name: str, download_url: str, file_count: int
        ) -> int:
            """
            ...
            if pathlib.Path(f'{local_path}/hash.json').exists():
                try:
                    with open(
                        pathlib.Path(f'{local_path}/hash.json'), encoding='utf-8'
                    ) as file_hash:
                ...
                for file_name, hash_value in json.loads(file_hash_str).items():
                    if pathlib.Path(f'{local_path}/{file_name}').resolve().exists():
                        ...
                        try:
                            with open(
                                pathlib.Path(f'{local_path}/{file_name}').resolve(), 'rb'
                            ) as file:
            ...
            for value in input_dat.clone_list.variants:
                if isinstance(value, dict):
                    ...
                    if config.user_input.verbose:  # type: ignore[unreachable]
                        warning_given = True
                        printwrap(
                            f'{Font.warning}* The following variants entry isn\'t an object and will be skipped:',
                            'error',
                        )
                        eprint(f'\n  {value}{Font.end}')
                ...
                for title_type in title_types:
                    ...
                    if title_type in value:
                        CloneListTools.process_variants(
                            value[title_type],
                            title_type[:-1],
                            value,
                            processed_titles,
