...
from modules.titletools import Removes, TraceTools
...
def clone_list_variants(
    ...
    deleted_titles: set[tuple[DatNode, str]],
    ...
    Args:
        ...
        clone_list_group_contents (dict[str, Any]): An entire group's contents from the
            clone list, including titles, compilations, and supersets.
        ...
        deleted_titles (set[tuple[DatNode, str]]): When titles listed in a clone list are
            processed, a new title is created to make the relevant changes. The original
            titles are added to this set, which is iterated over after this function has
            completed to remove the old titles from processed_titles.
        ...
        is_includes (bool, optional): Set to `True` when processing includes. Is only used
            to produce reliable reporting when performing a trace.
    ...
    for variant_title in variant_titles:
        ...
        if name_type == 'regex':
            ...
            for full_name in quick_lookup['full_name_index']:
                if pattern2string(re.compile(variant_name.lower()), full_name.lower()):
                    found_titles = found_titles | quick_lookup['full_name_index'][full_name]

        elif name_type == 'full':
            if variant_name in quick_lookup['full_name_index']:
                found_titles = quick_lookup['full_name_index'][variant_name]
        elif name_type == 'short':
            if variant_name.lower() in quick_lookup['short_name_index']:
                found_titles = quick_lookup['short_name_index'][variant_name.lower()]
        elif name_type == 'regionFree':
            if variant_name in quick_lookup['region_free_name_index']:
                found_titles = quick_lookup['region_free_name_index'][variant_name]
                    ...
                    if isinstance(variant_title['ignore'], bool):
                        if variant_title['ignore']:
                            if title in processed_titles[title.group_name]:
                                ...
                                removed_titles.clone_list_ignores.add(title)
                    else:
                ...
                for old_group_name in old_group_names:
                    if title in processed_titles[old_group_name]:
                        if variant_type == 'title' or variant_type == 'superset':
                            new_title: DatNode = deepcopy(title)
                            ...
                            for old_group_name in old_group_names:
                                if title in processed_titles[old_group_name]:
                                    deleted_titles.add((title, old_group_name))
                            ...
                            if 'filters' in variant_title:
                                # Check if at least one `conditions` and `results` keys exist
