...
def disable_incompatible_checkbox(
    ...
    checkbox_disable: tuple[qtw.QCheckBox, ...],
...
def enable_go_button(main_window: Any) -> None:
    ...
    if not disabled:
        ...
        main_window.ui.buttonGo.setToolTip(
            qtc.QCoreApplication.translate('MainWindow', f'{message}', None)
        )
...
def get_system_name(dat_file_path: str, config: Config) -> str:
    ...
    with open(pathlib.Path(dat_file_path), 'rb') as file:
        ...
        if (
            ...
            regex_search_str = pattern2string(
                re.compile('<name>(.*?)</name>'), str(header), group_number=1
            )
        ...
        # Clean up the name
        system_name = TitleTools.replace_invalid_characters(
            re.sub(' \\(Retool.*?\\)', '', system_name).replace('&amp;', '&'),
            config,
            is_header_detail=True,
        )
        ...
        if b'<url>' in header:
            system_url = pattern2string(
                re.compile('<url>(.*?)</url>'), str(header), group_number=1
            ).strip()

        if b'<homepage>' in header:
            system_homepage = pattern2string(
                re.compile('<homepage>(.*?)</homepage>'), str(header), group_number=1
            ).strip()

        if b'<comment>' in header:
            system_comment = pattern2string(
                re.compile('<comment>(.*?)</comment>'), str(header), group_number=1
            ).strip()

        if b'author' in header:
            system_author = pattern2string(
                re.compile('author.*([rR]edump.org)'), str(header), group_number=1
            ).strip()

        system_name = format_system_name(
            system_name, config, system_url, system_homepage, system_comment, system_author
        )
...
def move_list_items(
    origin_list_widget: qtw.QListWidget,
    destination_list_widget: qtw.QListWidget,
...
def order_list_items(list_widget: qtw.QListWidget, direction: str) -> None:
    ...
    selected_items: list[qtw.QListWidgetItem] = list(
        dict(sorted(selected_items_dict.items())).values()
    )
    ...
    if selected_items:
        if direction == 'up':
            ...
            if top_row <= 1:
                ...
                all_items = (
                    remainder_items[: top_row - 1] + selected_items + remainder_items[top_row - 1 :]
                )
        elif direction == 'down':
...
def set_path(
    parent: Any, current_path: str, label: qtw.QLabel, parent_attr: str, input_type: str = 'folder'
) -> None:
    """
    ...
    Args:
        ...
        current_path (str): What the path is currently set to.
