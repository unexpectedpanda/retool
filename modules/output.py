...
class WriteFiles:
    ...
    def output(
        ...
        log: tuple[dict[str, set[str]], set[DatNode]],
        config: Config,
        input_dat: Dat,
        removes: Removes,
    ) -> None:
            ...
            for region in region_order:
                ...
                if region_processed_titles:
                    WriteFiles.write_dat(
                        region_processed_titles, config, input_dat, timestamp, '', f' ({region})'
                    )
        else:
        ...
        if config.user_input.output_remove_dat:
            ...
            for titles in processed_titles.values():
                for title in titles:
                    if title.cloneof:
                        ...
                        title.cloneof = ''
                        if title.group_name not in removed_titles and not config.user_input.legacy:
                            removed_titles[title.group_name] = set()
            ...
            if removed_titles:
                if config.user_input.output_region_split:
                    ...
                    WriteFiles.write_dat(
                        removed_titles, config, input_dat, timestamp, ' (Removed titles)'
                    )
    ...
    def write_dat(
        ...
        for title in sorted(final_titles, key=lambda x: x.full_name):
            ...
            if title.cloneof:
                if config.user_input.legacy:
                    ...
                    if config.user_input.local_names:
                        # Fast search
                        clone_titles = TitleTools.find_title(
                            title.cloneof,
                            'full',
                            processed_titles,
                            set(),
                        ...
                        for clone_title in clone_titles:
                            if clone_title.local_name:
                                ...
                                if final_name == clone_title.local_name:
                                    ...
                                    dat_xml.append(
                                        f'\t<{node_name} name="{html.escape(final_name, quote=False)}" cloneof="{html.escape(clone_title.local_name, quote=False)}">\n'
                                    )
                            else:
                                dat_xml.append(
                                    f'\t<{node_name} name="{html.escape(final_name, quote=False)}" cloneof="{html.escape(title.cloneof, quote=False)}">\n'
                                )
                    else:
                        dat_xml.append(
                            f'\t<{node_name} name="{html.escape(final_name, quote=False)}" cloneof="{html.escape(title.cloneof, quote=False)}">\n'
            ...
            for rom in title.roms:
                ...
                if not (config.user_input.no_mia and mia == 'mia="yes"'):
        ...
        if config.user_input.original_header:
            ...
            final_xml.extend(
                [
                    ...
                    f'\t\t<description>{html.escape(input_dat.name, quote=False)} ({config.stats.file_count:,}){config.user_input.user_options}{excludes} ({input_dat.version}) (Retool {retool_version}){output_file_region}{output_file_removes}</description>\n',
        ...
        try:
            if config.stdout:
                ...
                with open(
                    pathlib.Path(input_dat.output_filename), 'w', encoding='utf-8'
                ) as output_file:
    ...
    def write_log(
        ...
        if (
            ...
            if title_names_with_clones:
                log_file_contents.append('* TITLES WITH CLONES\n')
            if titles_without_clones:
                log_file_contents.append('* TITLES WITHOUT CLONES\n')
            if removes.add_ons_removes:
                log_file_contents.append('* ADD-ON REMOVES\n')
            if removes.applications_removes:
                log_file_contents.append('* APPLICATION REMOVES\n')
            if removes.audio_removes:
                log_file_contents.append('* AUDIO REMOVES\n')
            if removes.bad_dumps_removes:
                log_file_contents.append('* BAD DUMP REMOVES\n')
            if removes.bios_removes:
                log_file_contents.append('* BIOS AND OTHER CHIP REMOVES\n')
            if removes.bonus_discs_removes:
                log_file_contents.append('* BONUS DISC REMOVES\n')
            if removes.coverdiscs_removes:
                log_file_contents.append('* COVERDISC REMOVES\n')
            if removes.demos_removes:
                log_file_contents.append('* DEMO, KIOSK, AND SAMPLE REMOVES\n')
            if removes.educational_removes:
                log_file_contents.append('* EDUCATIONAL REMOVES\n')
            if removes.games_removes:
                log_file_contents.append('* GAME REMOVES\n')
            if removes.manuals_removes:
                log_file_contents.append('* MANUAL REMOVES\n')
            if removes.mia_removes:
                log_file_contents.append('* MIA REMOVES\n')
            if removes.multimedia_removes:
                log_file_contents.append('* MULTIMEDIA REMOVES\n')
            if removes.pirate_removes:
                log_file_contents.append('* PIRATE REMOVES\n')
            if removes.preproduction_removes:
                log_file_contents.append('* PREPRODUCTION REMOVES\n')
            if removes.promotional_removes:
                log_file_contents.append('* PROMOTIONAL REMOVES\n')
            if removes.unlicensed_removes:
                log_file_contents.append('* UNLICENSED REMOVES\n')
            if removes.video_removes:
                log_file_contents.append('* VIDEO REMOVES\n')
            if removes.clone_list_removes:
                log_file_contents.append('* CLONE LIST REMOVES\n')
            if removes.language_removes:
                log_file_contents.append('* LANGUAGE REMOVES\n')
            if removes.region_removes:
                log_file_contents.append('* REGION REMOVES\n')
            if removes.global_excludes:
                log_file_contents.append('* GLOBAL EXCLUDES\n')
            if removes.system_excludes:
                log_file_contents.append('* SYSTEM EXCLUDES\n')
            if removes.global_filter_removes:
                log_file_contents.append('* GLOBAL POST FILTER REMOVES\n')
            if removes.system_filter_removes:
                log_file_contents.append('* SYSTEM POST FILTER REMOVES\n')
        ...
        try:
            with open(
                pathlib.Path(f'{output_filename} log.txt'), 'w', encoding='utf-8'
            ) as output_file:
