...
class IncludeExcludeTools(object):
            ...
            for title in titles:
                ...
                for exclude_category in [x for x in exclude_categories if x[0] in title.categories]:
                    ...
                    if (
                        ...
                        if not any(d.full_name == title.full_name for d in exclude_titles):
                            ...
                            category_remove_name = f'{category.lower()}_removes'.replace(' ' , '_').replace('-', '_')
                ...
                if not config.user_input.no_overrides:
                    if config.system_exclude:
                        if (
                            IncludeExcludeTools.user_override_match(title, config.system_exclude)
                            and not IncludeExcludeTools.user_override_match(title, config.system_include)):
                    ...
                    if config.global_exclude:
                        if (
                            ...
                            and not IncludeExcludeTools.user_override_match(title, config.global_include)
                            and not IncludeExcludeTools.user_override_match(title, config.system_include)):
    ...
    def includes(processed_titles: dict[str, set[DatNode]], original_titles: dict[str, set[DatNode]], original_titles_with_clonelist: dict[str, set[DatNode]], config: Config, removes: Removes) -> dict[str, set[DatNode]]:
        ...
        for titles in original_titles.values():
            for title in titles:
                ...
                if (
                    IncludeExcludeTools.user_override_match(title, config.global_include)
    ...
    @staticmethod
    def post_filters(processed_titles: dict[str, set[DatNode]], config: Config, removes: Removes) -> dict[str, set[DatNode]]:
        """ Works through a dict of DatNodes and removes nodes that match user criteria.
    ...
    @staticmethod
    def user_override_match(title: DatNode, filter_list: list[str], post_filter: bool = False, cloneof_check: bool = False) -> bool:
...
class TraceTools(object):
    ...
    def trace_title(trace_reference: str, variable: list[str] = [], title_set: set[DatNode] = set(), keep_remove: bool = False) -> None:
        ...
        if trace_reference == 'REF0045': message = 'ACTION: Excluded due to a system override:'
        if trace_reference == 'REF0046': message = 'ACTION: Excluded due to a global override:'
        if trace_reference == 'REF0047': message = 'ACTION: Included due to a system override:'
        if trace_reference == 'REF0048': message = 'ACTION: Included due to a global override:'
        if trace_reference == 'REF0049': message = f'ACTION: Removed due to user language filters: {variable[0]}'
        if trace_reference == 'REF0050': message = 'Would have been a system override exclude, but was cancelled out by a system include:'
        if trace_reference == 'REF0051': message = 'Would have been a global override exclude, but was cancelled out by a global include:'
        if trace_reference == 'REF0052': message = 'Would have been a global override exclude, but was cancelled out by a system include:'
        ...
        if trace_reference == 'REF0066': message = 'ACTION: Included due to being related to a system or global override:'
        ...
        if trace_reference == 'REF0099': message = 'ACTION: Kept due to system post filter:'
        if trace_reference == 'REF0100': message = 'ACTION: Kept due to global post filter:'
        if trace_reference == 'REF0101': message = f'{Font.bold}[{variable[0]}]{Font.end} Group after fallback user language priority check:'
        if trace_reference == 'REF0102': message = f'{Font.bold}[{variable[0]}]{Font.end} Group after fallback higher priority region check:'
        if trace_reference == 'REF0103': message = f'{Font.bold}[{variable[0]}]{Font.end} Group after non-superset clones removed:'
        if trace_reference == 'REF0104': message = f'{Font.bold}[{variable[0]}]{Font.end} Group after fallback user language priority check:'
