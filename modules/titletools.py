...
class IncludeExcludeTools(object):
    ...
    @staticmethod
    def excludes(processed_titles: dict[str, list[DatNode]], config: Config, removes: Removes) -> dict[str, list[DatNode]]:
        ...
        Returns:
            `dict[str, list[DatNode]]`: A dictionary of DatNodes with titles excluded
    ...
    @staticmethod
    def filter_languages(processed_titles: dict[str, list[DatNode]], config: Config, removes: Removes) -> dict[str, list[DatNode]]:
        ...
        Returns:
            `dict[str, list[DatNode]]`: A dictionary of DatNodes with titles filtered
            based on languages.
        """
        ...
        for group_name, titles in temp_dict.items():
            ...
            processed_titles[group_name] = list(supported_languages)
    ...
    @staticmethod
    def filter_regions(processed_titles: dict[str, list[DatNode]], config: Config, removes: Removes) -> dict[str, list[DatNode]]:
        ...
        for group_name, titles in temp_dict.items():
            ...
            processed_titles[group_name] = list(supported_regions)
    ...
    @staticmethod
    def includes(processed_titles: dict[str, list[DatNode]], original_titles: dict[str, list[DatNode]], original_titles_with_clonelist: dict[str, list[DatNode]], config: Config, removes: Removes) -> dict[str, list[DatNode]]:
        ...
        Args:
            ...
            of DatNodes, originally populated from the input DAT and actively being worked
            on by Retool.
            `original_titles (dict[str, list[DatNode]])`: A dictionary of DatNodes that
        ...
        Returns:
             `dict[str, list[DatNode]]`: A dictionary of DatNodes with titles added based
             on user criteria.
        """
        ...
        if any(x for x in include_titles if x.exclude_include_related):
            ...
            for include_title in include_titles:
                if include_title.exclude_include_related:
                    found_include_title: list[DatNode] = TitleTools.find_title(include_title.full_name, 'full', processed_titles, set(), config, deep_search=True)

                    for title in original_titles_with_clonelist[found_include_title[0].group_name]:
                         if not (
        ...
        # of the title
        for include_title in include_titles:
            found_current_titles: list[DatNode] = TitleTools.find_title(include_title.full_name, 'full', processed_titles, set(), config, deep_search=True)
            ...
            if include_title.group_name not in processed_titles:
                processed_titles[include_title.group_name] = []

            processed_titles[include_title.group_name].append(include_title)
...
class TitleTools(object):
            ...
            for title in title_set:
                if title_group in ','.join([TitleTools.get_group_name(x, config) for x in title.contains_titles]).lower():
                    ...
                    for individual_title in title.contains_titles:
                        ...
                        virtual_title.full_name = f':V: {individual_title} ({", ".join(title.regions)}){virtual_languages} • {title.full_name}'
    ...
    def find_title(name: str, name_type: str, titles: dict[str, list[DatNode]], missing_titles: set[str], config: Config, deep_search: bool = False) -> list[DatNode]:
        ...
        Returns:
            `list[DatNode]`: A list of titles that match the search terms.
        """
...
class TraceTools(object):
    ...
    def trace_title(trace_reference: str, variable: list[str] = [], title_set: set[DatNode] = set(), keep_remove: bool = False) -> None:
        ...
        if trace_reference == 'REF0019': message = f'{Font.bold}[{variable[0]}]{Font.end} {Font.success_bold}ACTION: Clone assignments:'
