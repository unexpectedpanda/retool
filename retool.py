...
__version__: str = str(f'{const.VERSION_MAJOR}.{const.VERSION_MINOR}')
...
def main(gui_input: UserInput | None = None) -> None:
    ...
    if 'description' in config.config_file_content:
        if 'minimumVersion' in config.config_file_content['description']:
            ...
            minimum_version(min_version, const.CONFIG_FILE, gui_input, config)
    ...
    if not (
        ...
        while not (download_updates == 'y' or download_updates == 'n'):
            printwrap(
                ...
                f'to download them? (y/n) > {Font.end}',
                'no_indent',
            )
        ...
        for i, dat_file in enumerate(dat_files):
            ...
            if not input_dat.end:
                ...
                if config.user_input.filter_languages:
                    processed_titles = IncludeExcludeTools.filter_languages(
                        processed_titles, config, removes
                    )

                # Filter regions
                processed_titles = IncludeExcludeTools.filter_regions(
                    processed_titles, config, removes
                )
                ...
                if not config.user_input.no_overrides:
                    if config.global_include or config.system_include:
                        ...
                        processed_titles = IncludeExcludeTools.includes(
                            processed_titles,
                            input_dat.contents_dict,
                ...
                if config.global_filter or config.system_filter:
                    processed_titles = IncludeExcludeTools.post_filters(
                        processed_titles, config, removes
                    )
                ...
                if not config.user_input.trace:
                    ...
                    if config.stats.final_count != 0:
                        ...
                        printwrap(
                            f'{Font.warning}* No titles in the input DAT match your preferences. No DAT file has been created.{Font.end}',
                            'error',
                        )
                        if input_type == 'file':
                            if gui_input:
